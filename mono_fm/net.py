...
class MonoFM(nn.Module):
    def __init__(self, options):
        super(MonoFM, self).__init__()
        self.opt = options
    ...
    def forward(self, inputs):
        ...
        if self.training:
            ...
            if self.global_step % 10 == 0:
                ...
                disp_resized_np = disp.squeeze().data.cpu().numpy()[0, :, :]

                vmax = np.percentile(disp_resized_np, 95)
                plt.imsave('./result_align.jpg', disp_resized_np, cmap='magma', vmax=vmax)
