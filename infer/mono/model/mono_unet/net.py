...
class mono_unet(nn.Module):
    def __init__(self, options):
        super(mono_unet, self).__init__()
        self.opt = options
    ...
    def featureL2Norm(self, feature):
        epsilon = 1e-6
        norm = torch.pow(torch.sum(torch.pow(feature, 2), 1) + epsilon, 0.5).unsqueeze(1).expand_as(feature)
        return torch.div(feature, norm)

    def compute_perceptional_loss(self, pred, target):
        f1 = self.extractor(pred)[0]
        f2 = self.extractor(target)[0]
        f1 = self.featureL2Norm(f1)
        f2 = self.featureL2Norm(f2)
        loss = -(f1 * f2).sum(1, True)
        return loss
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            # for frame_id in self.opt.frame_ids[1:]:
            #     pred = outputs[("color", frame_id, scale)]
            #     perceptional_losses.append(self.compute_perceptional_loss(pred, target))
            ...
            # loss_dict[('min_perceptional_loss', scale)] = min_perceptional_loss.mean() / len(self.opt.scales)
