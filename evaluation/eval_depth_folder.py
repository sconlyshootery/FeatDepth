#!/usr/bin/env python
# -*- coding:utf-8 -*-
# Author: Yukun(kunyu@deepmotion.ai)


from __future__ import absolute_import, division, print_function
import os
import cv2
import sys
import math
import numpy as np
from PIL import Image
from mmcv import Config
#import open3d as o3d
import torch
from torchvision import transforms

sys.path.append('.')
from core.model.registry import MONO
from core.model.model_airbus.layers import disp_to_depth
from data_module.dataset.utils import readlines, compute_errors
from data_module.dataset.kitti_dataset import KITTIRAWDataset

cv2.setNumThreads(0)  # This speeds up evaluation 5x on our unix systems (OpenCV 3.3.1)
STEREO_SCALE_FACTOR = 36.0
MIN_DEPTH = 1e-3
MAX_DEPTH = 100


def depth_err_image(depth_est, depth_gt):
    """
    Calculate the error map between disparity estimation and disparity ground-truth
    hot color -> big error, cold color -> small error
    Inputs:
        disp_est: numpy array, disparity estimation map in (Height, Width) layout, range [0,255]
        disp_gt:  numpy array, disparity ground-truth map in (Height, Width) layout, range [0,255]
    Outputs:
        disp_err: numpy array, disparity error map in (Height, Width, 3) layout, range [0,255]
    """
    depth_shape = depth_gt.shape

    # error color map with interval (0, 0.1875, 0.375, 0.75, 1.5, 3, 6, 12, 24, 48, inf)/3.0
    # different interval corresponds to different 3-channel projection
    cols = np.array([
        [12.0, float("inf"), 49, 54, 149],
        [8.0, 12.0, 69, 117, 180],
        [5.0, 8.0, 116, 173, 209],
        [3.0, 5.0, 171, 217, 233],
        [2.0, 3.0, 224, 243, 248],
        [1.2, 2.0, 254, 224, 144],
        [0.9, 1.2, 253, 174, 97],
        [0.6, 0.9, 244, 109, 67],
        [0.3, 0.6, 215, 48, 39],
        [0.0, 0.3, 165, 0, 38]
    ])

    E = np.abs(depth_est - depth_gt)

    # based on error color map, project the E within [cols[i,0], cols[i,1]] into 3-channel color image
    depth_err = np.zeros((depth_shape[0], depth_shape[1], 3), dtype='uint8')
    for c_i in range(cols.shape[0]):
        for i in range(depth_shape[0]):
            for j in range(depth_shape[1]):
                if depth_gt[i, j] != 0 and E[i, j] >= cols[c_i, 0] and E[i, j] <= cols[c_i, 1]:
                    depth_err[i, j, 0] = int(cols[c_i, 2])
                    depth_err[i, j, 1] = int(cols[c_i, 3])
                    depth_err[i, j, 2] = int(cols[c_i, 4])

    return depth_err


def log10(x):

    """Convert a new tensor with the base-10 logarithm of the elements of x. """
    return torch.log(x.float()) / math.log(10)


def evaluate(pred_disp, gt_depth):

    gt_height = gt_depth.shape[0]
    gt_width = gt_depth.shape[1]
    pred_disp = cv2.resize(pred_disp, (gt_width, gt_height))
    pred_depth = 1 / pred_disp

    mask = np.logical_and(gt_depth > MIN_DEPTH, gt_depth < MAX_DEPTH)
    pred_depth_m = pred_depth[mask]
    gt_depth_m = gt_depth[mask]
    ratio = np.median(gt_depth_m) / np.median(pred_depth_m)
    pred_depth *= ratio
    pred_depth_m *= ratio

    metric = compute_errors(gt_depth_m, pred_depth_m)


# err_image = depth_err_image(pred_depth, gt_depth)
    
    print(metric)

# return metric, err_image

    return metric, metric


def depth2point(depth, color_img):

    K = np.array([[0.9, 0, 0],
                 [0, 1.736, 0],
                 [0, 0, 1]])

    K[:, 0] = K[:, 0] * 512
    K[:, 1] = K[:, 1] * 256

    points = []
    colors = []
    for i in range(0, depth.shape[0], 2):
        for j in range(0, depth.shape[1], 2):
            if depth[i][j] > 10:
                continue
            x = j*depth[i][j] / 10.0
            y = i*depth[i][j] / 10.0
            z = depth[i][j] / 10.0
            points.append(np.array([x, y, z]))
            colors.append(color_img[i][j] / 255.0)
    points = np.array(points)
    points = np.linalg.inv(K).dot(points.transpose())
    colors = np.array(colors)

    return points, colors


def main(cfg, image_path, save_path, model_path, gt_path):

    resize = transforms.Resize((256, 512), interpolation=Image.ANTIALIAS)
    to_tensor = transforms.ToTensor()

    model = MONO.module_dict[cfg.model['name']](cfg.model)
    checkpoint = torch.load(model_path)
    model.load_state_dict(checkpoint['state_dict'], strict=True)
    model.cuda()
    model.eval()

    imgs = os.listdir(image_path)
    metrics = []
    print('running depth evaluation...')

    if not os.path.exists(save_path):
        os.mkdir(save_path)

    for imgn in imgs:

        gt = os.path.join(gt_path, imgn[:-4]+'.png')
        img_save_depth = os.path.join(save_path, imgn)

        img = os.path.join(image_path, imgn)
        img_write = cv2.imread(os.path.join(image_path, imgn))
        img = Image.open(img).convert('RGB')
        img = resize(img)
        gt_depth = cv2.imread(gt, -1) / 256.0
    #    gt_depth = cv2.resize(gt_depth, (512, 256))

#        img.save(os.path.join(save_path, img[:-4]+'ori.jpg'))
        img = to_tensor(img).unsqueeze(0).cuda()

        with torch.no_grad():
            output = model.DepthEncoder(img)
            output = model.DepthDecoder(output)
            disp = output[("disp", 0, 0)]

            pred_disp, depth = disp_to_depth(disp, 0.1, 100)
            depth = depth
            depth = depth.cpu()[:, 0].numpy()[0]
            pred_disp = pred_disp.cpu()[:, 0].numpy()[0]

        depth = cv2.resize(depth, (gt_depth.shape[1], gt_depth.shape[0]))
        mask = np.logical_and(gt_depth > MIN_DEPTH, gt_depth < MAX_DEPTH)
        gt_depth_m = gt_depth[mask]
        pred_depth_m = depth[mask]
        ratio = np.median(gt_depth_m) / np.median(pred_depth_m)

        mask_zero = gt_depth == 0
        gt_depth = (gt_depth - gt_depth.min()) / (gt_depth.max() - gt_depth.min()) * 255.0
        gt_depth = np.uint8(gt_depth)

        im_color = cv2.applyColorMap(gt_depth, cv2.COLORMAP_JET)
        im_color[mask_zero] = 0

        depth *= ratio

        depth_color = draw_img_with_measurement(img_write, depth, 60)
        depth_color_2 = draw_img_with_measurement(img_write, depth, 100)
        pred_depth_color = np.vstack([depth_color, depth_color_2])
        cv2.imwrite(img_save_depth, depth_color)

 #      cv2.imwrite('color_bar.png', color_bar_c)

#        cv2.imwrite('color_' + imgn, img_write)
#        cv2.imwrite('color_sub' + imgn, im_color_pred1)
#        cv2.imwrite('color_whole' + imgn, im_color_pred2)
#        cv2.imshow('bar', color_bar_c)
#        cv2.waitKey(0)

        # points, colors = depth2point(depth, img_write)
        #
        # pcd = o3d.geometry.PointCloud()
        # pcd.points = o3d.utility.Vector3dVector(points.transpose())
        #
        # pcd.colors = o3d.utility.Vector3dVector(colors)
        # o3d.visualization.draw_geometries([pcd])

        # im_color = cv2.applyColorMap(depth, cv2.COLORMAP_HSV)
        # cv2.imshow('color', im_color)
        # cv2.waitKey(0)

        # metric, err = evaluate(pred_disp, gt_depth)
        # metrics.append(metric)

#        cv2.imwrite(img_save_err, err)

#    mean_errors = np.array(metrics).mean(0)

#    print("\n" + ("{:>}| " * 7).format("abs_rel", "sq_rel", "rmse", "rmse_log", "a1", "a2", "a3"))
#    print(("&{:.3f} " * 7).format(*mean_errors.tolist()) + "\\\\")
    print("\n-> Done!")


def draw_img_with_measurement(img, img_depth, depth_range=40):
    img = cv2.resize(img, (800, 256))
    img_depth = cv2.resize(img_depth, (800, 256))

    mask = img_depth > depth_range
    img_depth[img_depth > depth_range] = depth_range
    img_depth = (img_depth - img_depth.min()) / (img_depth.max() - img_depth.min()) * 255.0

    color_depth = cv2.applyColorMap(np.uint8(img_depth), cv2.COLORMAP_JET)

    color_depth[mask] = 0

    color_bar = np.zeros([256, 30])
    for i in range(color_bar.shape[0]):
        color_bar[i] = 256 - i

    color_bar = cv2.applyColorMap(np.uint8(color_bar), cv2.COLORMAP_JET)

    show_img = np.zeros([img.shape[0] + 20, img.shape[1] * 2 + 100, 3])
    show_img[10:img.shape[0] + 10, :img.shape[1], :] = img
    show_img[10:img.shape[0] + 10, img.shape[1]:2 * img.shape[1], :] = color_depth
    show_img[10:img.shape[0] + 10, 2 * img.shape[1]: 2 * img.shape[1] + 30, :] = color_bar

    for i in range(9):
        show_img = cv2.arrowedLine(show_img, (img.shape[1] * 2 + 40, img.shape[0] - img.shape[0] // 8 * i + 10),
                                   (img.shape[1] * 2 + 30, img.shape[0] - img.shape[0] // 8 * i + 10),
                                   (0, 0, 255), 1, 1, 0, 0.3)

        show_img = cv2.putText(show_img, '%d' % (depth_range // 8 * i),
                               (img.shape[1] * 2 + 42, img.shape[0] - img.shape[0] // 8 * i + 10),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)

    show_img = cv2.putText(show_img, '(m)', (img.shape[1] * 2 + 60, img.shape[0] // 2),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)

    show_img = np.uint8(show_img)

    return show_img

if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description='KITTI Evaluation toolkit')
    parser.add_argument('--weight_path', type=str,
                        default='/home/kun/Desktop/Airbus/weights/kitti.pth',
                        help='model weight path')

    parser.add_argument('--cfg_path', type=str,
                        default='/home/kun/Desktop/Airbus/core/config/airbus_mono.py',
                        help='config of model')

    parser.add_argument('--save_path', type=str, default='/home/kun/Desktop/Airbus_kitti',
                        help='ground truth depth path')

    parser.add_argument('--img_path', type=str, default='/home/kun/Dataset/test/image_02',
                        help='image path')

    args = parser.parse_args()

    IMAGE_PATH = args.img_path
    SAVE_PATH = args.save_path
    MODEL_PATH = args.weight_path
    CFG_PATH = args.cfg_path
    GT_PATH = '/data/test/groundtruth'

    cfg = Config.fromfile(CFG_PATH)

    main(cfg, IMAGE_PATH, SAVE_PATH, MODEL_PATH, GT_PATH)

