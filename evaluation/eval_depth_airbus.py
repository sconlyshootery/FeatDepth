...
def main(cfg, image_path, gt_path, model_path):
    ...
    if not os.path.exists('./evaluation/depth_eval_airbus'):
        os.mkdir('./evaluation/depth_eval_airbus')

    for img in imgs:
        
        print(img)         
          
        gt = os.path.join(gt_path, img[:-4]+'.png')
        img_save_err = os.path.join('./evaluation/depth_eval_airbus', img[:-4]+'_err.jpg')
        img_save_depth = os.path.join('./evaluation/depth_eval_airbus', img[:-4]+'_depth.jpg')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gt_path', type=str, default='/data/test/depth',
                        help='ground truth depth path')

    parser.add_argument('--img_path', type=str, default='/data/test/img',
                        help='image path')
