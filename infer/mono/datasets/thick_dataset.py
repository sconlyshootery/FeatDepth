...
class MonoDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        if not is_train:
            self.gt_depths = np.load(gt_depth_path, allow_pickle=True, fix_imports=True, encoding='latin1')["data"]
    ...
    def __getitem__(self, index):
        ...
        frame_idxs = copy.deepcopy(self.frame_idxs)
        try:
            frame_idxs.remove('s')#[-1,0,1]
        except:
            pass

        idx_max = max(frame_idxs)+1#2
        frame_idxs.append(idx_max)
        idx_min = min(frame_idxs)-1#-2
        frame_idxs.append(idx_min)
        addition = 0
        try:
            self.get_color(folder,
                           frame_index + idx_max,
                           side,
                           do_flip)
        except:
        ...
        for i in frame_idxs:#[-2,-1,0,1,2]
            inputs[("color", i, -1)] = self.get_color(folder,
                                                      frame_index + i + addition,
                                                      side,
                                                      do_flip)
        ...
        if "s" in self.frame_idxs:
            for i in self.frame_idxs:
                if i != 's':#[-1,0,1]
                    ...
                    inputs[("color", str(i)+"s", -1)] = self.get_color(folder,
                                                                       frame_index + i + addition,
                                                                       other_side,
                                                                       do_flip)
