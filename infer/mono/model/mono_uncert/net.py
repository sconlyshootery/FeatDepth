...
"""
This model is almost the same with baseline, it is only used for analysis
"""

@MONO.register_module
class mono_uncert(nn.Module):
    def __init__(self, options):
        super(mono_uncert, self).__init__()
        self.opt = options
    ...
    def forward(self, inputs, k):
        ...
        loss_dict = self.compute_losses(inputs, outputs, k)
    ...
    def compute_losses(self, inputs, outputs, k):
        loss_dict = {}
        for scale in self.opt.scales:
            """
            initialization
            """
            disp = outputs[("disp", 0, scale)]*k
            outputs[("disp", 0, scale)] = outputs[("disp", 0, scale)]*k
            ...
            outputs[('min_reconstruct_loss', scale)] = min_reconstruct_loss
